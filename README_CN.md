...
* 使用 `skm publish` 将公钥上传到 GitHub、GitLab 或 Bitbucket
...
### 将公钥上传到 GitHub、GitLab 或 Bitbucket

`skm publish`（别名 `pub`）会通过对应平台的 API 上传公钥，省去手动复制粘贴
到网页 UI 的步骤。未指定别名时会使用当前激活的密钥。
...
% skm publish --github --dry-run                 # 预览，不实际上传
```

整个流程只会读取公钥，私钥不会被任何环节碰到。
...
3. 命令行回退（仅在以上都没有找到 token 时尝试）
   * GitHub:    `gh auth token`（需先 `gh auth login`；scope 需包含 `admin:public_key`）
   * GitLab:    `glab auth token`
   * Bitbucket: 无 —— 请使用 Bitbucket app password（scope: *Account: Write*）

若已有 `gh` 登录但缺少所需 scope：
...
通过 `--url` 指向 GitHub Enterprise 或自托管的 GitLab：
...
`publish` 会先拉取该平台上已有的公钥列表，按规范化形式（类型 + base64，忽略
末尾注释）与本地公钥比对。若发现重复，会打印提示并以 0 退出 —— 重复执行命令
是安全的：
...
默认使用 `skm-<alias>-<hostname>-<YYYYMMDD>`，便于日后在平台 UI 中辨认。
可通过 `--title` 覆盖：
